  ...
  return (
    <section className="surface-card rounded-[1.6rem] border border-white/70 p-3 sm:p-4">
      <div className="space-y-2 border-b border-border/80 pb-3">
        ...
        <p className="text-sm leading-7 text-muted-foreground">{copy.subtitle}</p>
      </div>
        ...
        return (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              />
              <span className="font-display text-xs font-bold tracking-tight text-foreground">{option.label}</span>
            </div>
...
function ChoiceField({ label, value, options, onChange }: ChoiceFieldProps) {
  return (
    <div className="space-y-1.5">
      <span className="font-display text-xs font-bold tracking-tight text-muted-foreground">{label}</span>
      <div className="flex flex-wrap gap-1.5">
        {options.map((option) => (
          <button
  ...
  return (
    <div className="rounded-[1.05rem] border border-border/80 bg-white/60 p-3 shadow-sm">
      <div className="mb-2 flex items-start justify-between gap-3">
        <p className="font-display text-[11px] font-bold uppercase tracking-[0.18em] text-muted-foreground">{label}</p>
        {action}
      </div>
  ...
  return (
    <div className="rounded-[1rem] border border-border/80 bg-white/72 px-3 py-2.5 shadow-sm">
      ...
      <div className="mt-1.5 flex items-center gap-1.5">
        ...
        <p className="font-display break-all text-xs font-bold tracking-tight text-foreground">{value}</p>
      </div>
    </div>
  );
}
...
function ModeButton({ active, label, onClick }: ModeButtonProps) {
  return (
    <button
      className={`font-display inline-flex cursor-pointer items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold tracking-tight transition ${
        active
